<script lang="ts">
  import { grantPool } from '../stores/character.js';
  import { setMembers, maxNumeric, memberLabel, type SetCategory } from '../character/index.js';
  ...
  const empty = $derived(sections.length === 0 && senses.length === 0 && speeds.length === 0);
